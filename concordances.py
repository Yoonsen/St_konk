...
def concordance(
    ...
    if words is None or words == '':
        return pd.DataFrame()  # exit condition
    else:
...
col1, col2, col3, col4, cold,  = st.columns([3,2,2,2,5])
...
with col2:
    antall = st.number_input("maks antall", min_value = 1, max_value = 5000, value = st.session_state.get('conc_numbers', 1000), key='conc_numbers', help="maks antall konkordanser")
with col3:
    kontekst = st.number_input('konkordansevindu', min_value = 1, max_value = 100, value = 25, key = 'conc_window', help="antallet ord konkordansen skal vise")
with col4:
    limit = st.number_input("antall konkordanser", min_value=5, max_value = 5000, value=50, help="hvor mange skal vises i gangen")
...
st.write("### Konkordanser med årstall og lenke")
st.write("Nedlastningsknapp under konkordansene")
st.write("---")
